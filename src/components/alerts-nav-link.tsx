...
export async function AlertsNavLink() {
  ...
  return (
    <Link
      href="/alerts"
      className="relative flex items-center text-mist-300 hover:text-horn-gold transition-colors"
      aria-label={count > 0 ? `Alerts (${count} active)` : "Alerts"}
    >
      <HornIcon className="h-5 w-5" />
      {count > 0 && (
        <span className="absolute -right-2.5 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-crimson px-1 text-[10px] font-semibold leading-none text-void">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </Link>
